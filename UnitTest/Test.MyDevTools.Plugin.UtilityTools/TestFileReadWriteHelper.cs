...
{
    [TestClass]
    public class TestFileReadWriteHelper
    {
...
一种泄露以 CBC 密码加密的数据的方式是对每个可能的密钥执行详尽搜索。 具体取决于用来执行加密的密钥大小，这种搜索即使是使用最快的计算机也非常耗时，因此不可行。 密钥大小更大，解密更难。 虽然从理论上来说，加密并未使攻击者检索加密数据变得不可能，但它确实增加了执行此操作的成本。 如果花费三个月的时间执行详尽搜索来检索仅在几天之内有意义的数据，则详尽搜索方法不切实际。";
        ...
        const String input2 = @"密钥加密算法使用单个密钥来加密和解密数据。 必须确保密钥不被未经授权的代理访问，因为具有此密钥的任意方均可使用此密钥解密你的数据或者加密自己的数据，而声称此数据来自于你。
...
";
        ...
        {
            //FileReadWriteHelper fileReadWriteHelper = new FileReadWriteHelper(Path.Combine(ApplicationCofig.AppDataDirectoryPath, "PasswordManagementTool"), "Test.data");

            IFileReadWriteHelper fileReadWriteHelper = Factory.CreateFileReadWriteHelper(Path.Combine(ApplicationCofig.AppDataDirectoryPath, "PasswordManagementTool"), "Test.data");
            fileReadWriteHelper.Write(input);
            fileReadWriteHelper.Write(input2);
            String outPut = fileReadWriteHelper.Read();

            Assert.IsNotNull(outPut);

            Console.WriteLine(outPut);
            Console.WriteLine(outPut.Length- input2.Length);
            ...
            for (int i = 0; i < outPut.Length; i++)
            {
                ...
                {
                    if (outPut[i] != input[i])
                        Console.Write("【{0}】<>【{1}】", outPut[i], input2[i]);
                }
                else
                {
                    Console.Write("outPut【{0}】【{1}】", outPut[i],i);
                }
            }
            Console.WriteLine("");

            Assert.IsTrue(outPut.Length>0);
            Assert.IsTrue(outPut.Length== input2.Length);
            Assert.IsTrue(outPut== input2);
        }
    }
}
