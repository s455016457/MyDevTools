...
{
    [TestClass]
    public class TestRijndaelCrypto
    {
...
一种泄露以 CBC 密码加密的数据的方式是对每个可能的密钥执行详尽搜索。 具体取决于用来执行加密的密钥大小，这种搜索即使是使用最快的计算机也非常耗时，因此不可行。 密钥大小更大，解密更难。 虽然从理论上来说，加密并未使攻击者检索加密数据变得不可能，但它确实增加了执行此操作的成本。 如果花费三个月的时间执行详尽搜索来检索仅在几天之内有意义的数据，则详尽搜索方法不切实际。";
        ...
        public void TestDecrypt()
        {
            ...
            String Key = keyValue.Key, Iv = keyValue.Value;
            Assert.IsNotNull(Key);
            Assert.IsNotNull(Iv);
            Assert.IsTrue(Key.Length > 0);
            Assert.IsTrue(Iv.Length > 0);

            Console.WriteLine("秘钥：{0}",Key);
            Console.WriteLine("偏移量：{0}",Iv);


            String encryptBody = RijndaelCrypto.Encryptor(input, Key, Iv);
            Assert.IsNotNull(encryptBody);
            Assert.IsTrue(encryptBody.Length > 0);
            Console.WriteLine("密文：{0}", encryptBody);
            String decryptBody = RijndaelCrypto.Decryptor(encryptBody, Key, Iv);

            Assert.IsNotNull(decryptBody);
            Assert.IsTrue(decryptBody.Length > 0);
            Console.WriteLine("明文：{0}", decryptBody);
            Assert.IsTrue(decryptBody.Equals(input));
        }
        
        [TestMethod]
        public void TestDecryptNoIv()
        {
            var key = RijndaelCrypto.CreateKeyNoIv();
            Assert.IsNotNull(key);
            Assert.IsTrue(key.Length > 0);
            Console.WriteLine("秘钥：{0}", key);

            String encryptBody = RijndaelCrypto.Encryptor(input, key);
            Assert.IsNotNull(encryptBody);
            Assert.IsTrue(encryptBody.Length > 0);
            Console.WriteLine("密文：{0}", encryptBody);

            String decryptBody = RijndaelCrypto.Decryptor(encryptBody, key);
            Assert.IsNotNull(decryptBody);
            Assert.IsTrue(decryptBody.Length > 0);
            Console.WriteLine("明文：{0}", decryptBody);
            Assert.IsTrue(decryptBody.Equals(input));
