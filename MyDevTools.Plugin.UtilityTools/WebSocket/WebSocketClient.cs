...
namespace MyDevTools.Plugin.UtilityTools.WebSocket
{
    ...
    public class WebSocketClient : IDisposable
    {
        ...
        public WebSocketClient(string Name, string Ip, int Port, string SubProtocol)
        {
           ...
           Console.WriteLine("创建WebSocket客户端");
        }
        #endregion
        ...
        public async Task OpenAsync()
        {
            ...
            {
               Console.WriteLine("WebSocket客户端已打开，不能再次打开");
                return;
            }
           ...
           Console.WriteLine("WebSocket客户端与服务器握手成功！");
        }
        ...
        public async Task SendMassage(string str)
        {
            if (ClientWebSocket.State != WebSocketState.Open)
            {
               Console.WriteLine("WebSocket客户端与服务器未握手成功，不能发送消息！");
                return;
            }
        ...
        public async Task ReceiveMessageAsync()
        {
            ...
            {
               Console.WriteLine($"连接被关闭：服务器关闭码【{webSocketReceiveResult.CloseStatus.Value}】，原因【{webSocketReceiveResult.CloseStatusDescription}】");
